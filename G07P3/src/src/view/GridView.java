...
public class GridView {
	...
	public GridView(int x, int y) {
		...
		startButton.addActionListener(e -> {
			...
			grid = buttonsToGrid(x, dimensionY, buttons);
			Cromosoma.tableroGlobal = grid;

			// ejecutar el algoritmo genetico
			ag.run();

			Cromosoma mejorHistorico = ag.getMejorHistorico();
	...
	private void finalButtonGrid(int x, int y, int[][] grid) {
		...
		buttonPanel.setLayout(new GridLayout(x, y));
		buttons = new JButton[x][y];
		buttons = gridToButtons(x, y, grid);
		for (int i = 0; i < x; i++) {
			for (int j = 0; j < y; j++) {
				buttonPanel.add(buttons[i][j]);
			}
			...
			for (int j = 0; j < y; j++) {
				switch (grid[i][j]) {
					case 0:
						...
						break;
					case 1:
						res[i][j].setBackground(lightGreen);
						break;
					default:
						res[i][j].setBackground(darkgrey);
						break;
				}
			}
		}
